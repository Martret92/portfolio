...
## V4 DuckyArena flagship

DuckyArena is the Home's second proof point and represents “The Experience.” Its presentation connects hidden team strategy, authoritative realtime combat and durable results to visible player feedback while keeping the collaborative academic foundation explicit.

The composition uses three existing redacted evidence captures: combat is the dominant arena frame, with character and lane selection providing distinct supporting context. Green, straw, gold and orange establish the project identity, while BLUE and RED accents retain team meaning. A single non-looping CSS arena reveal gives the dominant frame controlled energy; reduced motion presents the complete static frame immediately. The asymmetric desktop composition becomes a straightforward image-and-evidence sequence on smaller screens. Images remain lazy-loaded below the Hero and QuestBoard, the existing combat `srcset` serves a smaller mobile candidate, and V4 adds no JavaScript or dependency. DuckyArena's case study, QuestBoard and DevData remain structurally unchanged.

## V5 Selected Work and Stack

Selected Work closes the project narrative with a quieter DevData Generator treatment beneath the two flagships. One authentic optimized screenshot supports a compact static flow from configuration to the shared `generatedData` result, paired with the browser-only boundary and localized case-study/repository links. The Professional Portfolio is not added as a separate project because it has no equivalent registered case-study evidence.

Stack translates the existing capability content into three evidence-led tracks: Backend, Frontend and Engineering. Each combines a short responsibility statement, a concise technology list and one grounded evidence sentence without rankings, percentages or logo-cloud treatment. Three desktop columns become two-plus-one at tablet widths and one ordered column on mobile. V5 uses no animation, JavaScript or dependency; the DevData image remains lazy-loaded and unchanged. About, Experience, Education, CV and Contact remain deferred to later milestones.

## V6 About, CV and Contact
